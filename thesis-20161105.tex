...
the definition of the Lie algebra and the exponential and logarithm maps. Later, to describe
optimisation on matrix manifolds, the general definition of a Riemannian manifold, along with
the connection to Lie Groups is presented.
...
the words. Given two word-matrices $g, h$, we first map into their Lie Algebra
via $\log(g), \log(h)$. This is now an inner product space $\mathfrak g$, so we then
take the dot product $\log(g) \cdot \log(h)$.
